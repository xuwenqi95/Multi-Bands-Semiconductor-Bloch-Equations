...
def difft(Ek,k): # calculate dy/dt
...
class SBE:
    def __init__(self,d,VB,CB,dipole,k,E0field,E1field,T2,T1,time,Vkq):
        N1=len(CB)
        N2=len(VB)
        self.d=d   #lattice constant
        self.VB=VB #valence bands
        self.CB=CB #conduction bands
        
        self.bandN=N1+N2              #Number of bands
        ...
        self.dipole=dipole#dipole matrix element
        self.Efield=E0field+E1field#total Electric field
        self.E0=E0field#probe field
        self.T2=T2#dephasing time
        self.T1=T1#relaxation time
        self.time=time#simulation time axis
        self.freqs =fftshift(fftfreq(time.size, time[1] - time[0]))   #generate frequency
        self.energy=self.freqs*2*pi*27.21 #frequency to eV
        self.k=k#k axis
        ...
        self.Vgv=difft(self.VB,self.k)#group velocity
        self.Vgc=difft(self.CB,self.k)
        self.Ek=np.vstack((self.VB,self.CB))                       #Energy of CB and VB
        self.Vg=np.vstack((self.Vgv,self.Vgc))#group velocity
        self.Vkq=Vkq #Coulomb matrix
   
        
    def calculateddt(self): #calculate energy difference and dephasing term
    ...
    def showEnergy(self): #plot the bandstructure
        ...
        plt.xlabel('k(π/d)')
        plt.show()
    def solve(self): #solve pde through solve_ivp
        self.calculateddt()
    ...
    def calculate(self,Filter):#calculate Pt,Jt,Pw,Jt and Absorption
        y=self.sol.y.T
