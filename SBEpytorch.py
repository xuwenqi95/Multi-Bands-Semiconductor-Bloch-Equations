# -*- coding: utf-8 -*-
"""
Created on Wed Jan 18 13:38:04 2023
...
from finiteSBEtorch import finiteSBE
...
class SBE:
    def __init__(self,d,VB,CB,ddt,dipole,k,E0field,E1field,T2,T1,time,Vkq):
        N1=len(CB)
        N2=len(VB)
        self.d=d
        self.VB=VB
    ...
    def solve(self):
        ...
        self.y0=torch.from_numpy(self.y0).cuda()
        self.T1=torch.tensor([self.T1]).cuda()
        self.dk=torch.tensor([self.dk]).cuda()
        self.dipole=torch.from_numpy(self.dipole).cuda()
        self.ddt=torch.from_numpy(self.ddt).cuda()
        self.Vkq=torch.from_numpy(self.Vkq).cuda()
        self.k=torch.from_numpy(self.k).cuda()


        self.parameter=[self.bandN,self.kN,self.T1,self.dk,self.time,self.Efield,self.dipole,self.ddt,self.MP,self.Vkq,self.k,self.hindex,self.eindex]
        
        self.tensortime=torch.from_numpy(self.time).cuda()
        ...
        # self.sol=solve_ivp(finiteSBE, (self.time[0],self.time[-1]),self.y0, t_eval=self.time,atol=1e-10,rtol=1e-12,args=(self.parameter)) #use solve_ivp to solve the PDE, change the atol and rtol to control the accuracy

        self.sol= odeint(lambda t,y: finiteSBE(t,y,self.parameter),self.y0,self.tensortime,atol=1e-10,rtol=1e-12)
        # self.one=finiteSBE(self.tensortime[5000],self.y0,self.parameter)
